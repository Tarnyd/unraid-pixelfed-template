...
# Added git, nodejs, and npm which are required to fetch and build the app
...
# ffi is required for the new VIPS image processing in v0.12.7
RUN docker-php-ext-configure gd --with-freetype --with-jpeg --with-webp \
    && docker-php-ext-install -j$(nproc) \
    bcmath pcntl zip gd exif intl pdo_mysql ffi
...
# libvips42 and libffi-dev are critical here for the compiled PHP extensions to work
RUN apt-get update && apt-get install -y --no-install-recommends \
    libpng-dev libjpeg-dev libfreetype6-dev libzip-dev zlib1g-dev libicu-dev \
    libmagickwand-dev libwebp-dev libcurl4-openssl-dev libexif-dev libxml2-dev libssl-dev libonig-dev \
    libvips42 libffi-dev \
